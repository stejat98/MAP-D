...
# (a) UKB reverse observational: FDR < 0.05 OR stratum Bonferroni (0.05 / M proteins)
# (b) Subset with STEP trial P < 0.05 (hallmark-specific trial arm; see below)
# (c) Subset with reverse MR significant: nominal Rev IVW P-value < 0.05
#
  ...
  for (si in seq_along(strata)) {
    ...
    rev_sub <- rev_sub %>%
      mutate(
        ...
        bonf_ok = p.value < bonf,
        obs_sig = fdr_ok | bonf_ok
      )

    n_obs <- sum(rev_sub$obs_sig, na.rm = TRUE)
    ...
    rows_out[[length(rows_out) + 1L]] <- tibble(
      ...
      M_proteins_tested_reverse_obs = n_m,
      Bonferroni_threshold_reverse_obs = bonf,
      N_UKB_reverse_obs_FDR_or_Bonf = n_obs,
      ...
      MR_note = "Rev MR: nominal IVW P < 0.05 (not Bonferroni/FDR); MR not stratified by glycemia"
    )
  }
}
