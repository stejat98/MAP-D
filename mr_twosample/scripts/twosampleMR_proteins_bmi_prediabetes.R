# TwoSampleMR Analysis: Validated Proteins -> BMI (Prediabetes Strata)
# This script performs two-sample Mendelian Randomization to test
# causal effects of Step 1 & 2 validated proteins on BMI in prediabetes population
...
cat("=== TwoSampleMR: Validated Proteins -> BMI (Prediabetes Strata) ===\n\n")
...
# -----------------------------------------------------------------------------
# 5. Load BMI GWAS outcome data (Prediabetes Strata)
# -----------------------------------------------------------------------------
cat("\nStep 5: Loading BMI GWAS outcome data (Prediabetes Strata)...\n")

# Read BMI GWAS results for prediabetes strata
bmi_gwas <- fread(cmd = "zcat results/BMI_prediabetes_all_chr.regenie.gz")
cat(sprintf("  Total SNPs in BMI GWAS (prediabetes): %d\n", nrow(bmi_gwas)))
...
outcome_data <- data.frame(
  ...
  outcome = "BMI_prediabetes_strata",
  id.outcome = "BMI_prediabetes_strata",
...
write.csv(mr_results, file.path(output_dir, "MR_proteins_BMI_prediabetes.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "MR_proteins_BMI_prediabetes.csv")))

# Save heterogeneity results
write.csv(het_results, file.path(output_dir, "heterogeneity_proteins_BMI_prediabetes.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "heterogeneity_proteins_BMI_prediabetes.csv")))

# Save pleiotropy results
write.csv(pleio_results, file.path(output_dir, "pleiotropy_proteins_BMI_prediabetes.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "pleiotropy_proteins_BMI_prediabetes.csv")))

# Save harmonized data
write.csv(dat_harmonized, file.path(output_dir, "harmonized_data_proteins_BMI_prediabetes.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "harmonized_data_proteins_BMI_prediabetes.csv")))

# Save leave-one-out if available
if (!is.null(loo_results)) {
  write.csv(loo_results, file.path(output_dir, "leaveoneout_proteins_BMI_prediabetes.csv"), row.names = FALSE)
  cat(sprintf("  Saved: %s\n", file.path(output_dir, "leaveoneout_proteins_BMI_prediabetes.csv")))
}
