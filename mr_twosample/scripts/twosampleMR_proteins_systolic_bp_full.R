# TwoSampleMR Analysis: Validated Proteins -> Systolic BP (Full Strata)
# This script performs two-sample Mendelian Randomization to test
# causal effects of Step 1 & 2 validated proteins on Systolic BP
...
cat("=== TwoSampleMR: Validated Proteins -> Systolic BP (Full Strata) ===\n\n")
...
# -----------------------------------------------------------------------------
# 5. Load Systolic BP GWAS outcome data (Full Strata)
# -----------------------------------------------------------------------------
cat("\nStep 5: Loading Systolic BP GWAS outcome data (Full Strata)...\n")

outcome_gwas <- fread(cmd = "zcat results/systolic_BP_full_all_chr.regenie.gz")
cat(sprintf("  Total SNPs in Systolic BP GWAS: %d\n", nrow(outcome_gwas)))
...
cat(sprintf("  SNPs found in Systolic BP GWAS: %d\n", nrow(outcome_filtered)))
...
outcome_data <- data.frame(
  ...
  outcome = "systolic_BP_full_strata",
  id.outcome = "systolic_BP_full_strata",
...
write.csv(mr_results, file.path(output_dir, "MR_proteins_systolic_BP_full.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "MR_proteins_systolic_BP_full.csv")))

write.csv(het_results, file.path(output_dir, "heterogeneity_proteins_systolic_BP_full.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "heterogeneity_proteins_systolic_BP_full.csv")))

write.csv(pleio_results, file.path(output_dir, "pleiotropy_proteins_systolic_BP_full.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "pleiotropy_proteins_systolic_BP_full.csv")))

write.csv(dat_harmonized, file.path(output_dir, "harmonized_data_proteins_systolic_BP_full.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "harmonized_data_proteins_systolic_BP_full.csv")))

if (!is.null(loo_results)) {
  write.csv(loo_results, file.path(output_dir, "leaveoneout_proteins_systolic_BP_full.csv"), row.names = FALSE)
  cat(sprintf("  Saved: %s\n", file.path(output_dir, "leaveoneout_proteins_systolic_BP_full.csv")))
}
