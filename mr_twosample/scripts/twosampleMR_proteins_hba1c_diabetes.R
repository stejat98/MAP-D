# TwoSampleMR Analysis: Validated Proteins -> HbA1c (Diabetes Strata)
# This script performs two-sample Mendelian Randomization to test
# causal effects of Step 1 & 2 validated proteins on HbA1c in diabetes population
...
cat("=== TwoSampleMR: Validated Proteins -> HbA1c (Diabetes Strata) ===\n\n")
...
# -----------------------------------------------------------------------------
# 5. Load HbA1c GWAS outcome data (Diabetes Strata)
# -----------------------------------------------------------------------------
cat("\nStep 5: Loading HbA1c GWAS outcome data (Diabetes Strata)...\n")

# Read HbA1c GWAS results for diabetes strata
hba1c_gwas <- fread(cmd = "zcat results/HbA1c_diabetes_all_chr.regenie.gz")
cat(sprintf("  Total SNPs in HbA1c GWAS (diabetes): %d\n", nrow(hba1c_gwas)))
...
outcome_data <- data.frame(
  ...
  outcome = "HbA1c_diabetes_strata",
  id.outcome = "HbA1c_diabetes_strata",
...
write.csv(mr_results, file.path(output_dir, "MR_proteins_HbA1c_diabetes.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "MR_proteins_HbA1c_diabetes.csv")))

# Save heterogeneity results
write.csv(het_results, file.path(output_dir, "heterogeneity_proteins_HbA1c_diabetes.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "heterogeneity_proteins_HbA1c_diabetes.csv")))

# Save pleiotropy results
write.csv(pleio_results, file.path(output_dir, "pleiotropy_proteins_HbA1c_diabetes.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "pleiotropy_proteins_HbA1c_diabetes.csv")))

# Save harmonized data
write.csv(dat_harmonized, file.path(output_dir, "harmonized_data_proteins_HbA1c_diabetes.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "harmonized_data_proteins_HbA1c_diabetes.csv")))

# Save leave-one-out if available
if (!is.null(loo_results)) {
  write.csv(loo_results, file.path(output_dir, "leaveoneout_proteins_HbA1c_diabetes.csv"), row.names = FALSE)
  cat(sprintf("  Saved: %s\n", file.path(output_dir, "leaveoneout_proteins_HbA1c_diabetes.csv")))
}
