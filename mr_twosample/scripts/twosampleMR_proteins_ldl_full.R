# TwoSampleMR Analysis: Validated Proteins -> LDL (Full Strata)
# This script performs two-sample Mendelian Randomization to test
# causal effects of Step 1 & 2 validated proteins on LDL
...
cat("=== TwoSampleMR: Validated Proteins -> LDL (Full Strata) ===\n\n")
...
# -----------------------------------------------------------------------------
# 5. Load LDL GWAS outcome data (Full Strata)
# -----------------------------------------------------------------------------
cat("\nStep 5: Loading LDL GWAS outcome data (Full Strata)...\n")

outcome_gwas <- fread(cmd = "zcat results/LDL_full_all_chr.regenie.gz")
cat(sprintf("  Total SNPs in LDL GWAS: %d\n", nrow(outcome_gwas)))
...
cat(sprintf("  SNPs found in LDL GWAS: %d\n", nrow(outcome_filtered)))
...
outcome_data <- data.frame(
  ...
  outcome = "LDL_full_strata",
  id.outcome = "LDL_full_strata",
...
write.csv(mr_results, file.path(output_dir, "MR_proteins_LDL_full.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "MR_proteins_LDL_full.csv")))

write.csv(het_results, file.path(output_dir, "heterogeneity_proteins_LDL_full.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "heterogeneity_proteins_LDL_full.csv")))

write.csv(pleio_results, file.path(output_dir, "pleiotropy_proteins_LDL_full.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "pleiotropy_proteins_LDL_full.csv")))

write.csv(dat_harmonized, file.path(output_dir, "harmonized_data_proteins_LDL_full.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "harmonized_data_proteins_LDL_full.csv")))

if (!is.null(loo_results)) {
  write.csv(loo_results, file.path(output_dir, "leaveoneout_proteins_LDL_full.csv"), row.names = FALSE)
  cat(sprintf("  Saved: %s\n", file.path(output_dir, "leaveoneout_proteins_LDL_full.csv")))
}
