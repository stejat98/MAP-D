#!/usr/bin/env Rscript
# =============================================================================
# Mendelian Randomization: DECODE cis-pQTLs for LEPR -> BMI (UKB full stratum)
#
# Exposure: Leptin Receptor (LEPR) cis-pQTLs from DECODE Iceland (SomaScan)
#   File: Proteomics_SMP_PC0_5400_52_LEPR_sLeptin_R_10032022.txt.gz
#   LEPR gene: chr1:65,420,652-65,641,559 (hg38)
#   Cis window: +/- 1Mb
#
...
# =============================================================================
# 1. Load DECODE LEPR pQTL data - extract cis region
# =============================================================================
...
cis_start <- lepr_start - cis_window
cis_end   <- lepr_end + cis_window

cat(sprintf("  LEPR gene: %s:%d-%d\n", lepr_chr, lepr_start, lepr_end))
cat(sprintf("  Cis window: %s:%d-%d (+/- %g Mb)\n", lepr_chr, cis_start, cis_end, cis_window / 1e6))

# Read full file and filter to cis region
cmd <- sprintf(
  "zcat %s | awk 'NR==1 || ($1==\"chr1\" && $2>=%d && $2<=%d)'",
...
if (nrow(cis_sig) == 0) {
  stop("No genome-wide significant cis-pQTLs found for LEPR in DECODE data!")
}
...
exposure_raw <- data.frame(
  ...
  exposure = "LEPR_DECODE_cis",
  id.exposure = "LEPR_DECODE_cis",
...
write.csv(dat_harmonized,
  file.path(output_dir, "harmonized_decode_LEPR_BMI_full.csv"),
  row.names = FALSE
)
...
cat(sprintf("  Exposure: LEPR (Leptin Receptor) - DECODE Iceland cis-pQTLs\n"))
...
write.csv(mr_results,
  file.path(output_dir, "MR_decode_LEPR_BMI_full.csv"),
  row.names = FALSE
)
...
if (nrow(dat_harmonized) > 1) {
  ...
  print(het)
  write.csv(het, file.path(output_dir, "heterogeneity_decode_LEPR_BMI_full.csv"),
    row.names = FALSE)
  cat("\n")
  ...
  if (nrow(dat_harmonized) >= 3) {
    ...
    print(pleio)
    write.csv(pleio, file.path(output_dir, "pleiotropy_decode_LEPR_BMI_full.csv"),
      row.names = FALSE)
    cat("\n")
  }
  ...
  print(loo)
  write.csv(loo, file.path(output_dir, "leaveoneout_decode_LEPR_BMI_full.csv"),
    row.names = FALSE)
  cat("\n")
  ...
  print(single)
  write.csv(single, file.path(output_dir, "singlesnp_decode_LEPR_BMI_full.csv"),
    row.names = FALSE)
  ...
  ggsave(file.path(output_dir, "scatter_decode_LEPR_BMI_full.png"),
    p_scatter[[1]], width = 8, height = 6, dpi = 150)
  ggsave(file.path(output_dir, "scatter_decode_LEPR_BMI_full.pdf"),
    p_scatter[[1]], width = 8, height = 6)
  cat("  Saved scatter plot\n")

  # Forest plot
  p_forest <- mr_forest_plot(single)
  ggsave(file.path(output_dir, "forest_decode_LEPR_BMI_full.png"),
    p_forest[[1]], width = 10, height = 6, dpi = 150)
  ggsave(file.path(output_dir, "forest_decode_LEPR_BMI_full.pdf"),
    p_forest[[1]], width = 10, height = 6)
  cat("  Saved forest plot\n")

  # Leave-one-out plot
  p_loo <- mr_leaveoneout_plot(loo)
  ggsave(file.path(output_dir, "leaveoneout_decode_LEPR_BMI_full.png"),
    p_loo[[1]], width = 10, height = 6, dpi = 150)
  ggsave(file.path(output_dir, "leaveoneout_decode_LEPR_BMI_full.pdf"),
    p_loo[[1]], width = 10, height = 6)
  cat("  Saved leave-one-out plot\n")

  # Funnel plot
  p_funnel <- mr_funnel_plot(single)
  ggsave(file.path(output_dir, "funnel_decode_LEPR_BMI_full.png"),
    p_funnel[[1]], width = 8, height = 6, dpi = 150)
  ggsave(file.path(output_dir, "funnel_decode_LEPR_BMI_full.pdf"),
  ...
  cat(sprintf("  Exposure (LEPR):  beta = %.4f, SE = %.4f\n", beta_exp, se_exp))
...
cat("Data sources:\n")
cat("  Exposure: DECODE Iceland SomaScan pQTLs (N = 35,652)\n")
cat("  Outcome:  UKB BMI full stratum (regenie)\n")
cat(sprintf("  Cis window: chr1:%d-%d (+/- 1Mb around LEPR)\n", cis_start, cis_end))
cat(sprintf("  N instruments: %d\n\n", nrow(dat_harmonized)))
