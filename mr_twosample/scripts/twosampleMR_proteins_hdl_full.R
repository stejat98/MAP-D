# TwoSampleMR Analysis: Validated Proteins -> HDL (Full Strata)
# This script performs two-sample Mendelian Randomization to test
# causal effects of Step 1 & 2 validated proteins on HDL
...
cat("=== TwoSampleMR: Validated Proteins -> HDL (Full Strata) ===\n\n")
...
# -----------------------------------------------------------------------------
# 5. Load HDL GWAS outcome data (Full Strata)
# -----------------------------------------------------------------------------
cat("\nStep 5: Loading HDL GWAS outcome data (Full Strata)...\n")

# Read HDL GWAS results
outcome_gwas <- fread(cmd = "zcat results/HDL_full_all_chr.regenie.gz")
cat(sprintf("  Total SNPs in HDL GWAS: %d\n", nrow(outcome_gwas)))

# Get SNPs we need from exposure
snps_needed <- unique(exposure_dat$SNP)
cat(sprintf("  SNPs needed from outcome: %d\n", length(snps_needed)))

# Filter GWAS for SNPs in exposure
outcome_filtered <- outcome_gwas %>% filter(ID %in% snps_needed)
cat(sprintf("  SNPs found in HDL GWAS: %d\n", nrow(outcome_filtered)))
...
outcome_data <- data.frame(
  ...
  outcome = "HDL_full_strata",
  id.outcome = "HDL_full_strata",
...
write.csv(mr_results, file.path(output_dir, "MR_proteins_HDL_full.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "MR_proteins_HDL_full.csv")))

write.csv(het_results, file.path(output_dir, "heterogeneity_proteins_HDL_full.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "heterogeneity_proteins_HDL_full.csv")))

write.csv(pleio_results, file.path(output_dir, "pleiotropy_proteins_HDL_full.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "pleiotropy_proteins_HDL_full.csv")))

write.csv(dat_harmonized, file.path(output_dir, "harmonized_data_proteins_HDL_full.csv"), row.names = FALSE)
cat(sprintf("  Saved: %s\n", file.path(output_dir, "harmonized_data_proteins_HDL_full.csv")))

if (!is.null(loo_results)) {
  write.csv(loo_results, file.path(output_dir, "leaveoneout_proteins_HDL_full.csv"), row.names = FALSE)
  cat(sprintf("  Saved: %s\n", file.path(output_dir, "leaveoneout_proteins_HDL_full.csv")))
}
