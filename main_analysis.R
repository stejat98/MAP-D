...
# 2. Protein-wide association studies (PWAS) using existing helper functions
...
#' Main analysis pipeline using existing PWAS helper functions
#' 
...
run_mapd_pipeline <- function(run_full_pipeline = FALSE, 
  ...
  # ==========================================================================
  # STEP 3: METABOLIC PHENOTYPE PWAS USING EXISTING HELPER FUNCTIONS
  # ==========================================================================
  ...
  # ==========================================================================
  # STEP 4: COMPLICATION OUTCOME PWAS USING EXISTING HELPER FUNCTIONS
  # ==========================================================================
  ...
  if (check_file_exists(step1_file, "STEP1 validation data")) {
    ...
    if (file.exists("src/validation.R")) {
      ...
      validation_results <- run_external_validation(all_pwas_results, results_directory)
      pipeline_results$validation <- validation_results
    } else {
      cat("Validation module not found - skipping validation step\n")
    }
...
create_analysis_summary <- function(pipeline_results, output_dir) {
  ...
  cat("\n", rep("=", 62), "\n", sep = "")
  cat("Analysis completed successfully using existing PWAS helper functions.\n")
