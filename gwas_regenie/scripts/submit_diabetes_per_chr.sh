...
PHENO_LIST="${SCRIPT_DIR}/pheno_list_diabetes_per_chr.txt"
...
sbatch \
  ...
  --mem=50G \
  --partition=short \
  --job-name=regenie_diabetes_perchr \
  --output=/n/groups/patel/sivateja/regenie_pipeline/slurm/regenie_diabetes_perchr_%A_%a.out \
  --error=/n/groups/patel/sivateja/regenie_pipeline/slurm/regenie_diabetes_perchr_%A_%a.err \
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "Monitor jobs:"
    echo "  squeue -u st320 -p short | grep regenie_diabetes_perchr"
    echo ""
    echo "Logs: /n/groups/patel/sivateja/regenie_pipeline/slurm/regenie_diabetes_perchr_*.out"
else
