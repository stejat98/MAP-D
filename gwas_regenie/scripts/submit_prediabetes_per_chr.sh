...
sbatch \
  ...
  --mem=50G \
  --partition=short \
  --job-name=regenie_prediabetes_perchr \
  --output=/n/groups/patel/sivateja/regenie_pipeline/slurm/regenie_prediabetes_perchr_%A_%a.out \
  --error=/n/groups/patel/sivateja/regenie_pipeline/slurm/regenie_prediabetes_perchr_%A_%a.err \
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "Monitor jobs:"
    echo "  squeue -u st320 -p short | grep regenie_prediabetes_perchr"
    echo ""
    echo "Logs: /n/groups/patel/sivateja/regenie_pipeline/slurm/regenie_prediabetes_perchr_*.out"
else
