#!/bin/bash
# Submit REGENIE jobs for prediabetes strata split by chromosome (to avoid timeouts)
...
PHENO_LIST="${SCRIPT_DIR}/pheno_list_prediabetes_per_chr_remaining.txt"

echo "=========================================="
echo "Submitting REGENIE jobs for prediabetes strata (split by chromosome)"
echo "=========================================="
...
sbatch \
  ...
  --mem=50G \
  --partition=short \
  --job-name=regenie_prediabetes_perchr_remaining \
  --output=/n/groups/patel/sivateja/regenie_pipeline/slurm/regenie_prediabetes_perchr_remaining_%A_%a.out \
  --error=/n/groups/patel/sivateja/regenie_pipeline/slurm/regenie_prediabetes_perchr_remaining_%A_%a.err \
  --export=PHENO_LIST_FILE="$PHENO_LIST" \
  "$REGENIE_SCRIPT"

if [ $? -eq 0 ]; then
    echo "  ✓ Prediabetes jobs submitted (split by chromosome, short queue)"
    echo ""
    echo "Summary:"
    ...
    echo ""
    echo "Monitor jobs:"
    echo "  squeue -u st320 -p short | grep regenie_prediabetes_perchr_remaining"
    echo ""
    echo "Logs: /n/groups/patel/sivateja/regenie_pipeline/slurm/regenie_prediabetes_perchr_remaining_*.out"
else
